...
Both presentation modes also contribute an **Open DSH CLI** footer action to the official sidebar, rendered above the plugin market with a terminal glyph. Clicking it asks the desktop Host through a same-origin loopback route to open the isolated DSH terminal for the active profile, so the packaged `dsh`, `pnpm`, and `node` shims stay reachable without leaving the application; a native error dialog reports an unavailable terminal just like the tray command. In the wide sidebar the two footer actions stack vertically with the CLI launcher first, while the collapsed rail keeps both as inline icons.
...
macOS and Linux replace Electron's default English menu bar with a Chinese application menu (`文件`/`编辑`/`视图`/`窗口`/`帮助`, plus the macOS application menu), so the window and system menu bars match the product locale. Windows keeps its existing menu-bar removal.

## Packaging

`yarn package:dir` creates an unpacked directory for the current host platform. The packaged-runtime gate rejects an application archive that omits the desktop update and terminal modules, the DSH CLI bootstrap, the bundled pnpm entry, or the physical deployment package. Electron Builder emits the root manifest, desktop runtime, and complete dependency tree under `app.asar.unpacked`; both Host profile boot and the CLI bootstrap use this physical tree so DSH profile-fallback symlinks never target a virtual ASAR directory. `build/app-icon.png` remains the unmodified iOS Default source and the Windows application icon. The build runs `scripts/generate-mac-app-icon.mjs` to center that artwork at 824 by 824 pixels on a transparent 1024 by 1024 canvas; macOS packaging and the live Dock both use the generated `build/app-icon-mac.png`. The build also runs `scripts/generate-linux-icons.mjs` to derive the standard 16-512px freedesktop hicolor set under `build/icons/`; Linux deb, rpm, and AppImage packaging installs those icons into `/usr/share/icons/hicolor/<size>x<size>/apps/` so desktop environments resolve the `Icon=` entry of the packaged `.desktop` file (a single oversized PNG would land in a non-standard size directory that launchers ignore). `build/tray-icon.svg` is the brand-blue tray source: the build derives a macOS template image that the system colors automatically and fixed brand-blue Windows and Linux tray images.
...
- The tray terminal exposes private `dsh`, `pnpm`, and `node` shims. Separately, the Host runtime exposes the bundled `pnpm` command on the current Electron process `PATH` for ambient compatibility and provides the managed `desktopPnpm` service; none of these commands are added to the system `PATH`, and the Linux terminal command depends on an installed terminal emulator (gnome-terminal, konsole, xfce4-terminal, kitty, alacritty, xterm, `x-terminal-emulator`, or `xdg-terminal-exec`).
