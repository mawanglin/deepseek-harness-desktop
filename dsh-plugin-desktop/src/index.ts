...
import {
  DESKTOP_TERMINAL_OPEN_PATH as CLI_LAUNCHER_TERMINAL_OPEN_PATH,
} from './desktop-cli-launcher-contract.ts'
import {
  handleDesktopTerminalOpenRequest as handleCliLauncherTerminalOpenRequest,
} from './desktop-cli-launcher-route.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.effect(
    () => ctx.webServer.register({
      ...
      handler: (req, res) => {
        if (rejectDesktopRequest(ctx, req, res)) return
        return handleCliLauncherTerminalOpenRequest(
          req,
          res,
          rendererOrigin,
          () => { runtime.openTerminal() },
          cause => {
            ctx.logger.error(`dsh-plugin-desktop: failed to open the desktop CLI terminal from the renderer: ${cause instanceof Error ? cause.message : String(cause)}`)
          },
        )
      },
    }),
    'dsh-plugin-desktop: desktop CLI launcher terminal open route',
  )
